#!/usr/bin/env python3
# BLS Employment Situation RSS Bootstrap / First-Enable v1 — seen-only snapshot.
...
ADAPTER_FILE = "collect-bls-empsit-rss.py"
ADAPTER_MODULE = "collect_bls_empsit_rss"
...
def item_identity(adapter, item):
    try:
        url = adapter.assert_bls_link(item.get("link") or item.get("guid") or "")
    except ValueError:
        url = ""
    guid = adapter.normalize_url(item.get("guid") or "")
    if guid:
        try:
            guid = adapter.assert_bls_link(guid)
        except ValueError:
            guid = ""
    return url, guid
...
def bootstrap(feed_url=None, fixture_path=None, store_root=None):
    ...
    for item in items:
        ...
        if not url:
            ...
            errors.append({
                "stage": "identity",
                "message": "missing or non-BLS url/link",
...
def main(argv):
    ...
    while i < len(argv):
        ...
        if argv[i] == "--store-root" and i + 1 < len(argv):
            ...
            i += 2
            continue
        sys.stderr.write("BLS_EMPSIT_BOOTSTRAP_FAIL\nunknown argument: " + argv[i] + "\n")
        return 2
