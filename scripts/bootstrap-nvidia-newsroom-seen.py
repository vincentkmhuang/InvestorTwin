#!/usr/bin/env python3
# NVIDIA Newsroom Bootstrap / First-Enable v1 — seen-only snapshot.
#
...
#
# Identity MUST match scripts/collect-nvidia-newsroom-rss.py exactly.
...
def load_nvidia_adapter():
    path = os.path.join(SCRIPTS_DIR, "collect-nvidia-newsroom-rss.py")
    spec = importlib.util.spec_from_file_location("collect_nvidia_newsroom_rss", path)
...
def item_identity(adapter, item):
    try:
        url = adapter.assert_nvidia_link(item.get("link") or item.get("guid") or "")
    except ValueError:
        url = ""
    guid = adapter.normalize_url(item.get("guid") or "")
    if guid:
        try:
            guid = adapter.assert_nvidia_link(guid)
        except ValueError:
            guid = ""
    return url, guid
...
def bootstrap(feed_url=None, fixture_path=None, store_root=None):
    ...
    for item in items:
        ...
        if not url:
            ...
            errors.append({
                "stage": "identity",
                "message": "missing or non-NVIDIA url/link",
...
def main(argv):
    ...
    while i < len(argv):
        ...
        if argv[i] == "--store-root" and i + 1 < len(argv):
            ...
            i += 2
            continue
        sys.stderr.write("NVIDIA_BOOTSTRAP_FAIL\nunknown argument: " + argv[i] + "\n")
        return 2
