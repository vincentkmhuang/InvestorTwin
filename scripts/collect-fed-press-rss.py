#!/usr/bin/env python3
# News Collect Foundation — Federal Reserve Press RSS adapter (Collect only).
# Fetch → parse → normalize → data/news/runs/<runId>/{raw,normalized,run.json}
# Reuses subject inference helpers from federal-reserve-official-adapter.py.
...
SOURCE_ID = "fed-press-rss"
SOURCE_LABEL = "Federal Reserve Press RSS"
DEFAULT_FEED_URL = "https://www.federalreserve.gov/feeds/press_all.xml"
USER_AGENT = "InvestorTwin-NewsCollect/026 (personal; non-commercial)"
ALLOWED_HOSTS = (
...
def load_fed_official_module():
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "federal-reserve-official-adapter.py")
    spec = importlib.util.spec_from_file_location("federal_reserve_official_adapter", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
...
def assert_fed_link(url):
    """Only Federal Reserve official HTTPS links may enter Collect."""
    value = normalize_url(url)
    if not value:
        return ""
    parsed = urllib.parse.urlparse(value)
    if parsed.scheme != "https":
        raise ValueError("Fed Press item link must be HTTPS: " + value)
    host = (parsed.hostname or "").lower()
    if host not in ALLOWED_HOSTS:
        raise ValueError("REJECT non-Federal Reserve link host: " + host)
    return value
...
def http_get_text(url, timeout=40):
    ...
    for verify in (True, False):
        try:
            ...
            # Attach status for live smoke callers via attribute on result string is awkward;
            # callers that need status re-fetch. Store last status on module for tests.
...
def item_to_news(item, collected_at, run_id, fed_official=None):
    url = assert_fed_link(item.get("link") or item.get("guid") or "")
    title = strip_html(item.get("title") or "")
    if fed_official is not None:
        title = fed_official.normalize_title(title) or title
    ...
    subject = None
    if fed_official is not None:
        subject = fed_official.infer_subject(title, summary)
    news = {
        "source": SOURCE_LABEL,
        "title": title,
...
def collect(feed_url=None, fixture_path=None, store_root=None):
    ...
    fed_official = load_fed_official_module()
    try:
    ...
    for item in items:
        try:
            news = item_to_news(item, collected_at, run_id, fed_official=fed_official)
            url = news.get("url") or ""
            guid = normalize_url(item.get("guid") or "")
            if guid:
                try:
                    guid = assert_fed_link(guid)
                except ValueError:
                    guid = ""
            if not url:
...
def main(argv):
    ...
    while i < len(argv):
        ...
        if argv[i] == "--store-root" and i + 1 < len(argv):
            ...
            i += 2
            continue
        sys.stderr.write("FED_PRESS_RSS_ADAPTER_FAIL\nunknown argument: " + argv[i] + "\n")
        return 2
