# Investor Twin Phase 2 Sprint 003 — Federal Reserve Official Source Adapter.
# Input: one Federal Reserve official article URL, or offline HTML + that URL.
...
def assert_fed_https(url):
    if not isinstance(url, str) or not url.strip():
        fail("missing Federal Reserve official URL")
    parsed = urllib.parse.urlparse(url.strip())
    if parsed.scheme != "https":
        fail("only HTTPS Federal Reserve official URLs are allowed")
    host = (parsed.hostname or "").lower()
    if host not in ALLOWED_HOSTS:
        fail("REJECT non-Federal Reserve official source: " + host)
...
def http_get_text(url):
    request = urllib.request.Request(url, headers={"User-Agent": "InvestorTwin-NewsAdapter/003"})
    try:
        ...
        fail("invalid Federal Reserve URL: HTTP " + str(exc.code))
    except urllib.error.URLError as exc:
        fail("invalid Federal Reserve URL: " + str(exc.reason))
    except TimeoutError:
        fail("invalid Federal Reserve URL: timeout")
    text = raw.decode("utf-8", errors="replace")
    if "<" not in text:
        fail("Federal Reserve article did not return HTML")
    return text
...
class ArticleParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if attrs.get("id", "").lower() == "article" or "article__time" in classes:
        ...
        if tag in ("title", "h1", "h3", "p"):
            self._capture = tag
            self._buf = []
            self._capture_class = classes
    ...
    def handle_endtag(self, tag):
        ...
        if self._capture == tag:
            ...
            classes = getattr(self, "_capture_class", "")
            if tag == "title" and text and not self.title:
                self.title = text
            elif tag in ("h1", "h3") and text:
                if not self.title or self.title.lower().startswith("federal reserve board"):
                    self.title = text
                if tag == "h3":
                    self._in_article = True
            elif tag == "p" and text:
                if "article__time" in classes and not self.published:
                    self.published = text
                    self._in_article = True
                elif self._in_article and "releasetime" not in classes and len(text) >= 40:
...
def normalize_title(title):
    ...
    for prefix in ("Federal Reserve Board - ", "The Fed - "):
...
def infer_subject(title, summary):
    ...
    if (
        "federal reserve" in hay
        or "fomc" in hay
        or "open market committee" in hay
        or "monetary policy" in hay
    ):
        return "Federal Reserve / FOMC / U.S. monetary policy"
    return None
...
def news_object(url, html_text):
    ...
    long_paras = [para for para in parser.paragraphs if len(para) >= 80]
    summary = " ".join(long_paras[:1]).strip()
    if not summary:
