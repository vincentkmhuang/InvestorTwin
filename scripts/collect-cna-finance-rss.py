#!/usr/bin/env python3
# News Collect Foundation v1 — CNA Finance RSS adapter (Collect only).
...
SOURCE_ID = "cna-finance-rss"
SOURCE_LABEL = "CNA Finance RSS"
DEFAULT_FEED_URL = "https://feeds.feedburner.com/rsscna/finance"
USER_AGENT = "InvestorTwin-NewsCollect/014 (personal; non-commercial)"
...
def normalize_url(url):
    ...
    if not value:
        return ""
    # Strip common tracking fragments only; keep path/query for identity.
...
def parse_published(value):
    ...
    try:
        ...
        # Keep original non-empty string rather than inventing a date.
        return raw
...
def child_text(node, names):
    ...
    for child in list(node):
        if local_name(child.tag) in wanted:
            ...
            if text:
                return text
            # Some feeds put URL in attributes or nested text.
...
def collect(feed_url=None, fixture_path=None, store_root=None):
    ...
    for item in items:
        try:
            ...
            # Drop None-valued forbidden accidental keys; keep subject null explicitly.
            ...
            # contract.fail raises SystemExit
...
def main(argv):
    ...
    while i < len(argv):
        ...
        if argv[i] == "--store-root" and i + 1 < len(argv):
            ...
            i += 2
            continue
        sys.stderr.write("CNA_RSS_ADAPTER_FAIL\nunknown argument: " + argv[i] + "\n")
        return 2
