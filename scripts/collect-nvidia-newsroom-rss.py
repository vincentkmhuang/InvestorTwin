#!/usr/bin/env python3
# News Collect Foundation — NVIDIA Newsroom RSS adapter (Collect only).
# Fetch → parse → normalize → data/news/runs/<runId>/{raw,normalized,run.json}
# Reuses subject/title helpers from nvidia-official-adapter.py.
...
SOURCE_LABEL = "NVIDIA Newsroom RSS"
DEFAULT_FEED_URL = "https://nvidianews.nvidia.com/rss.xml"
USER_AGENT = "InvestorTwin-NewsCollect/027 (personal; non-commercial)"
ALLOWED_HOSTS = (
    "blogs.nvidia.com",
    "nvidianews.nvidia.com",
)
# Newsroom feed can exceed 400KB; keep headroom for media namespaces.
...
def load_nvidia_official_module():
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nvidia-official-adapter.py")
    spec = importlib.util.spec_from_file_location("nvidia_official_adapter", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
...
def assert_nvidia_link(url):
    """Only NVIDIA official HTTPS links may enter Collect."""
    ...
    if host not in ALLOWED_HOSTS:
        raise ValueError("REJECT non-NVIDIA official link host: " + host)
    # Upgrade http→https for official hosts (feed occasionally uses http scheme).
    if parsed.scheme == "http":
        value = urllib.parse.urlunparse(("https",) + parsed[1:])
    elif parsed.scheme != "https":
        raise ValueError("NVIDIA Newsroom item link must be HTTPS: " + value)
    return value
...
def item_to_news(item, collected_at, run_id, nvidia_official=None):
    url = assert_nvidia_link(item.get("link") or item.get("guid") or "")
    title = strip_html(item.get("title") or "")
    if nvidia_official is not None:
        title = nvidia_official.normalize_title(title) or title
    ...
    subject = None
    if nvidia_official is not None:
        subject = nvidia_official.infer_subject(title, summary)
    if not subject and "nvidia" in (title + " " + summary).lower():
        subject = "NVIDIA"
    news = {
        "source": SOURCE_LABEL,
        "title": title,
...
def collect(feed_url=None, fixture_path=None, store_root=None):
    ...
    raw_name = "nvidia-newsroom-rss.xml"
    ...
    nvidia_official = load_nvidia_official_module()
    try:
    ...
    for item in items:
        try:
            news = item_to_news(item, collected_at, run_id, nvidia_official=nvidia_official)
            url = news.get("url") or ""
            guid = normalize_url(item.get("guid") or "")
            if guid:
                try:
                    guid = assert_nvidia_link(guid)
                except ValueError:
                    guid = ""
            if not url:
...
def main(argv):
    ...
    while i < len(argv):
        ...
        if argv[i] == "--store-root" and i + 1 < len(argv):
            ...
            i += 2
            continue
        sys.stderr.write("NVIDIA_NEWSROOM_RSS_ADAPTER_FAIL\nunknown argument: " + argv[i] + "\n")
        return 2
