#!/usr/bin/env python3
# News Collect Foundation — EIA Today in Energy RSS adapter (Collect only).
# Official feed: https://www.eia.gov/rss/todayinenergy.xml
...
SOURCE_ID = "eia-today-energy-rss"
SOURCE_LABEL = "EIA Today in Energy RSS"
DEFAULT_FEED_URL = "https://www.eia.gov/rss/todayinenergy.xml"
USER_AGENT = "InvestorTwin-NewsCollect/031 (personal; non-commercial)"
...
def assert_eia_link(url, base_url=None):
    value = normalize_url(url)
    if not value:
        return ""
    if value.startswith("/"):
        value = urllib.parse.urljoin("https://www.eia.gov", value)
    elif not urllib.parse.urlparse(value).scheme and base_url:
        value = urllib.parse.urljoin(base_url, value)
    parsed = urllib.parse.urlparse(value)
    host = (parsed.hostname or "").lower()
    if host not in ALLOWED_HOSTS:
        raise ValueError("REJECT non-EIA official link host: " + host)
    if parsed.scheme == "http":
        value = urllib.parse.urlunparse(("https",) + parsed[1:])
    elif parsed.scheme != "https":
        raise ValueError("EIA item link must be HTTPS: " + value)
    return value
...
def infer_subject(title, summary):
    ...
    if "petroleum" in hay or "crude" in hay or " oil" in hay:
        return "Petroleum"
    if "natural gas" in hay or "lng" in hay:
        return "Natural Gas"
    if "electric" in hay or "power" in hay:
        return "Electricity"
    return "EIA"


def item_to_news(item, collected_at, run_id, feed_url=None):
    url = assert_eia_link(item.get("link") or item.get("guid") or "", base_url=feed_url or DEFAULT_FEED_URL)
...
def collect(feed_url=None, fixture_path=None, store_root=None):
    ...
    raw_name = "eia-today-energy-rss.xml"
    ...
    for item in items:
        try:
            news = item_to_news(item, collected_at, run_id, feed_url=feed_url)
            url = news.get("url") or ""
            guid = normalize_url(item.get("guid") or "")
            if guid:
                try:
                    guid = assert_eia_link(guid, base_url=feed_url)
                except ValueError:
                    guid = ""
            if not url:
...
def main(argv):
    ...
    while i < len(argv):
        ...
        if argv[i] == "--store-root" and i + 1 < len(argv):
            ...
            i += 2
            continue
        sys.stderr.write("EIA_TODAY_ENERGY_RSS_ADAPTER_FAIL\nunknown argument: " + argv[i] + "\n")
        return 2
