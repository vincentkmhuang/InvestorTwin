#!/usr/bin/env python3
# News Collect Foundation — BLS CPI RSS adapter (Collect only).
# Official feed: https://www.bls.gov/feed/cpi.rss (Atom)
...
SOURCE_ID = "bls-cpi-rss"
SOURCE_LABEL = "BLS CPI News Release RSS"
DEFAULT_FEED_URL = "https://www.bls.gov/feed/cpi.rss"
...
def infer_subject(title, summary):
    ...
    if "cpi" in hay or "consumer price" in hay or "inflation" in hay:
        return "CPI"
    return "BLS"
...
def main(argv):
    ...
    while i < len(argv):
        ...
        if argv[i] == "--store-root" and i + 1 < len(argv):
            ...
            i += 2
            continue
        sys.stderr.write("BLS_CPI_RSS_ADAPTER_FAIL\nunknown argument: " + argv[i] + "\n")
        return 2
