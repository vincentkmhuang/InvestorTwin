...
$script:TempRoot = Join-Path $env:TEMP ('InvestorTwin-011-B-2-' + [guid]::NewGuid().ToString('N'))
...
function Reset-GlassBridgeResearch {
...
function Get-QueueHits {
  $queue = Read-JsonFile $script:TempQueuePath
  $list = @($queue.items)
  return ,@($list | Where-Object { $_.id -eq $Item.researchId })
}
...
try {
  ...
  if ([int]$respExisting.StatusCode -ne 200) {
    $fail1.Add(("HTTP {0} {1}" -f $respExisting.StatusCode, $respExisting.Body))
  }
  ...
  if ($hits.Count -lt 1) {
    $fail2.Add('queue has no item whose id equals monitoringTrigger.researchId')
  } elseif ([string]$hits[0].id -ne [string]$Item.researchId) {
    $fail2.Add("queue id=$($hits[0].id) expected $($Item.researchId)")
  }
  Add-TestResult 'TEST 2' ($fail2.Count -eq 0) ($fail2 -join "`n")

  $fail3 = New-Object System.Collections.Generic.List[string]
  if ($hits.Count -ne 1) { $fail3.Add("queue glass-bridge count=$($hits.Count) expected 1") }
  elseif ([string]$hits[0].addedFrom -ne 'Morning Brief') {
    $fail3.Add("addedFrom=$($hits[0].addedFrom) expected original Morning Brief")
  }
  Add-TestResult 'TEST 3' ($fail3.Count -eq 0) ($fail3 -join "`n")

  $fail4 = New-Object System.Collections.Generic.List[string]
  if ([int]$respExisting.StatusCode -ne 200) {
    $fail4.Add(("HTTP {0} {1}" -f $respExisting.StatusCode, $respExisting.Body))
  ...
  if ($casesHashAfter -ne $casesHashBefore) { $fail5.Add('investment-cases.json was written by monitoringTrigger') }
  foreach ($msg in @(
      ...
      (Assert-Unchanged 'case.status' $beforeExisting.caseStatus $afterExisting.caseStatus)
      (Assert-Unchanged 'positionPlaybook' $beforeExisting.positionPlaybook $afterExisting.positionPlaybook)
    )) {
  ...
  if ([int]$respMissing.StatusCode -ne 200) {
    $fail6.Add(("HTTP {0} {1}" -f $respMissing.StatusCode, $respMissing.Body))
  } else {
    $created = @(Get-QueueHits)
    if ($created.Count -ne 1) {
      $fail6.Add("expected one queue item for $($Item.researchId), got $($created.Count)")
    } elseif ($created[0].addedFrom -ne $Contract.expectedQueueSource) {
      $fail6.Add("addedFrom=$($created[0].addedFrom) expected $($Contract.expectedQueueSource)")
    }
  }
  Add-TestResult 'TEST 6' ($fail6.Count -eq 0) ($fail6 -join "`n")

  $respNoText = Invoke-Post '/api/cases' '{"id":"3363-glass-bridge","monitoringTrigger":{"researchId":"glass-bridge"}}'
  Add-TestResult 'TEST 7' ([int]$respNoText.StatusCode -eq 400) $(if ([int]$respNoText.StatusCode -eq 400) { '' } else { "HTTP {0} {1}" -f $respNoText.StatusCode, $respNoText.Body })

  $respNoResearchId = Invoke-Post '/api/cases' '{"id":"3363-glass-bridge","monitoringTrigger":{"text":"Glass Bridge 量產驗證是否確認"}}'
  Add-TestResult 'TEST 8' ([int]$respNoResearchId.StatusCode -eq 400) $(if ([int]$respNoResearchId.StatusCode -eq 400) { '' } else { "HTTP {0} {1}" -f $respNoResearchId.StatusCode, $respNoResearchId.Body })

  $respNullTrigger = Invoke-Post '/api/cases' '{"id":"3363-glass-bridge","monitoringTrigger":null}'
  Add-TestResult 'TEST 9' ([int]$respNullTrigger.StatusCode -eq 400) $(if ([int]$respNullTrigger.StatusCode -eq 400) { '' } else { "HTTP {0} {1}" -f $respNullTrigger.StatusCode, $respNullTrigger.Body })
}
catch {
